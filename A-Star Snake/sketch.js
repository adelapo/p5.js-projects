...
function getPath(apple) {
	...
	src = new QItem(head, 0, getDistance(head, apple)); // The snake's head is the source node
	...
	while (pq.items.length > 0 && pq.items.length < 400) { // Algorithm terminates if the queue is empty
		...
		for (var adjacentNode of adj) {
			...
			// and it's closer than previously seen,
			// then enqueue the adjacent node.
			
			// gscore is minItem.priority - getDistance(minElem, apple) + 1
			// hscore is getDistance(adjacentNode, apple)
			gScore = minItem.gScore + 1;
			if (gScore < pq.getPriorityOfArray(adjacentNode) - getDistance(adjacentNode, apple)) {
				qitem = new QItem(adjacentNode, gScore, getDistance(adjacentNode, apple));
				pq.enqueue(qitem);
...
function getDistance(squareA, squareB) {
	return abs(squareA[0] - squareB[0]) + abs(squareA[0] - squareB[0]);
}